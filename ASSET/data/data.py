...
def process_single_file(file_base):
    ...
    file_name = os.path.join(os.getcwd(), "pcp_use_cases","ASSET", "data", file_base + ".csv")
    ...
    for line in fp_in.readlines():
        ...
        # Grab the time from the cs file
        if line.strip()[0:6] == "#Total":
            total_runtime = float(line.strip().split(",")[1].strip())
            continue
...
if __name__ == "__main__":
    ...
    files_for_parse = [
       ["original",       "Original" ],
       ["precompute_log2", "precompute\nlog" ],
       ["early_loop_exit", "early loop\nexit"],
       ["reshape",      "reshape\ndata"  ],
       ["float32",      "consistent float32"                   ],
       ["cython_exp_opt", "cython\n fast exp"],
       ["final_mpi16",  "MPI on\n16 ranks" ]
        ]
    ...
    ("init"                     ,"init","#93EDAA"),
    ("diff"                     ,"diff","#65A375"),
    ("reshape"                  ,"reshape","#2B8041"),
    ("sum"                      ,"sum","#2B8061"),

    # Red color
    ("log_DU2"                  ,"log","#EB8571"),
    ("diag_log_DU2"             ,"ones","#E08571"),
    ("copy_log_DU2"             ,"copy","#f78571"),
